...
int wosign_sign(const char* data, const size_t len, char** signature, size_t* sig_len)
{
	...
	deepin_message(LOG_DEBUG,"Init [Success]\n");
	...
	deepin_message(LOG_DEBUG,"Login [Success]\n");
	...
	deepin_message(LOG_DEBUG,"Find key [Success]\n");
	...
	deepin_message(LOG_DEBUG,"Signature [Success]\n");
	...
	deepin_message(LOG_DEBUG,"Logout [Success]\n");
...
#define MAX_HANDLE_COUNT 255
    ...
    if(CKR_OK != ckrv || ulRetCount == 0) {
        deepin_message(LOG_ERROR, "no private key found: 0x%lx!!\n", ckrv);
        return -13;
    }
...
static char *wosign_do_sign(const char *data, const int len, size_t *sign_len)
{
	CK_RV rv;
	char *sign = NULL;
	size_t tmp_len = MAX_SM2_SIGN_LEN;

	rv = C_SignInit(_sign_info.hSession, &_sign_info.mechanism, _sign_info.hPriKey);
	if (CKR_OK != rv) {
		deepin_message(LOG_ERROR, "failed to init sm2 sign: %lu\n", rv);
		return NULL;
	}

	sign = (char*)calloc(MAX_SM2_SIGN_LEN, sizeof(char));
	if (!sign) {
		return NULL;
	}
	...
	if (rv != CKR_OK) {
		deepin_message(LOG_ERROR, "failed to sm2 sign: %lu\n", rv);
		free(sign);
		sign = NULL;
		return NULL;
	}
