...
void lsm_add_partition(const char* dir_name, int major, int minor)
{
	...
	pr_info("[%s] start to add dir_name: %s\n", __func__, dir_name);
	...
	pr_info("partition %s [%d, %d] added, comm[%d]: %s\n",
		part->root, major, minor, current->pid, current->comm?current->comm:"NULL");
	down_write(&rw_parts);
...
#endif
		pr_info("[%s] read size: real(%d), size: %d\n", __func__, *real_size, size);
...
int get_major_minor(const char* dir_name, unsigned int* major, unsigned int* minor)
{
	...
	pr_info("[%s] %s: dev(%u)\n", __func__, dir_name, path.dentry->d_sb->s_dev);
...
static int get_major_minor_from_sysfs(const char *dev_name, unsigned int* major, unsigned int* minor)
{
	...
	pr_info("[%s] sysfs major: %u, minor: %u, content: '%s'\n", __func__, ma, mi, content);
	kfree(content);
	content = NULL;
...
static void init_mounts_info(void)
{
	...
	pr_info("[%s] init mount: %d\n", __func__, _mount_init);
	if (_mount_init != 0)
		return;
	_mount_init = 1;
			  ...
			  unsigned long prot, unsigned long flags)
{
	...
	if (*root == 0)
		goto out;
	pr_info("[%s] first get root, cmd: %s, root: %s, major: %u, minor: %u, name: %s\n",
		__func__, cmd, root, major, minor, file->f_path.dentry->d_name.name);
